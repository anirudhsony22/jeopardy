# Jeopardy - Get relevant wikipedia document for your question

**Owner:** Anirudha Soni  
**Full Report:** [Project Report](https://docs.google.com/document/d/1hGXzrfwM0spRU-2zFPhOaqKzWD7p_7kgl-mSVvFfges/edit?usp=sharing)

## Project Details

This project aims to develop a system that retrieves the most relevant Wikipedia articles in response to user questions, leveraging a pipeline that processes and indexes a large corpus of data.

- **Data**: ~280,000 Wikipedia articles  
- **Tokens**: ~123 million  
- **Questions**: 100 (with answers)  
- **Evaluation Targets**:  
  - Recall@1 ≥ 40%  
  - MRR ≥ 50%

## Pipeline Overview

The system follows a multi-stage pipeline to process and retrieve relevant documents:

1. Preprocessing
2. Chunking
3. Embedding
4. Indexing
5. Merging
6. Re-ranking

### Faster Pipeline  
*Lower latency, slight loss in accuracy (54% Precision@1 and 64.59 MRR)*

 ![Pipeline Optimized](https://private-user-images.githubusercontent.com/44290537/451571219-d20c629b-e2f7-489b-aa3d-a6a5171e8730.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..xE7ENdcbpbNc2JLH02ltfSosM6Hlf0izIlSYubMXRaA)


### 🔍 Original Pipeline  
*More accurate (57% Precision@1 and 67.6 MRR), but slightly slower*

 ![Pipeline Old](https://private-user-images.githubusercontent.com/44290537/451571383-fa894419-b0d3-49b1-b80b-7ec86f527ee0.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..NWh5Xgvv7pYhkkRtYH-S4HAMVVToAjsuXFMbC16R50A)

## ⚠️ Notes

- Index files are **not included** in this repository due to storage constraints.
